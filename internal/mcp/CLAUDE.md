...
| `ToolGetStats`            | `get_stats`              | handlers.go:788        | Returns `{stats, accounts, vector_search?, backup?}` (handlers.go:746). `backup` is best-effort host-ops metadata (freshness of the external nightly backup script's marker file, mtime-based) — populated only when `ServeOptions.BackupStateDir` is set; see `BackupStatus`/`collectBackupStatus` (handlers.go). |
